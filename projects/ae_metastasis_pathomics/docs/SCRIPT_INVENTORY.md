...
## `scripts/build_ae_metastasis_submission_package.py`

Public-safe replacement for the local final-submission builder for the AE metastasis prediction manuscript. The local source script embedded the complete unpublished manuscript and submission metadata, so this repository version uses external markdown input and a placeholder template.

Key behavior:

- Reads `AE_METASTASIS_ROOT` or uses the current working directory.
...
- Reads `AE_METASTASIS_ROOT` for controlled output locations.
